...
void local_mapper::triangulate_with_two_keyframes(data::keyframe* keyfrm_1, data::keyframe* keyfrm_2,
    ...
    const float ratio_factor = 2.0f * std::max(keyfrm_1->scale_factor_, keyfrm_2->scale_factor_);
...
#pragma omp parallel for
    ...
    for (unsigned int j = 0; j < matches.size(); ++j) {
        ...
        const auto cos_stereo_parallax_1 = is_stereo_1
                                           ? std::cos(2.0 * atan2(camera_1->true_baseline_ / 2.0, keyfrm_1->depths_.at(idx_1)))
                                           : 2.0;
        const auto cos_stereo_parallax_2 = is_stereo_2
                                           ? std::cos(2.0 * atan2(camera_2->true_baseline_ / 2.0, keyfrm_2->depths_.at(idx_2)))
                                           : 2.0;
