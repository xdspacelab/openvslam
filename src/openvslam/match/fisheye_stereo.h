#ifndef OPENVSLAM_MATCH_FISHEYE_STEREO_H
#define OPENVSLAM_MATCH_FISHEYE_STEREO_H
...
class fisheye_stereo {
public:
    fisheye_stereo() = delete;

    fisheye_stereo(const camera::fisheye* camera, const std::vector<cv::Mat>& left_image_pyramid, const std::vector<cv::Mat>& right_image_pyramid,
    ...
    virtual ~fisheye_stereo() = default;
...
private:
    //! enumeration to control the behavior of rectify_patch_image()
    ...
    /**
 * Rectify images around point
 * @param point
 * @param img_side
 * @param half_patch_size: actual patch size is 2*patch_size + 1
 * @param patch
 */
    void rectify_patch_image(const cv::KeyPoint& keypt, const image_side& img_side, const cv::Size& half_patch_size, cv::Mat &patch) const;

    //! camera model
    const camera::fisheye* camera_;
    ...
    //! reference to undistort keypoints in left image
    const std::vector<cv::KeyPoint> undistort_keypts_left_;
    //! reference to undistort keypoints in right image
    const std::vector<cv::KeyPoint> undistort_keypts_right_;
...
#endif // OPENVSLAM_MATCH_FISHEYE_STEREO_H
