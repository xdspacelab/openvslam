...
publisher::publisher(const std::shared_ptr<openvslam::config>& cfg, openvslam::system* system,
        ...
        : system_(system),
          emitting_interval_(cfg->yaml_node_["SocketViewer.emitting_interval"].as<unsigned int>(15000)),
          image_quality_(cfg->yaml_node_["SocketViewer.image_quality"].as<unsigned int>(20)),
          client_(new socket_client(cfg->yaml_node_["SocketViewer.server_uri"].as<std::string>("http://127.0.0.1:3000"))) {
